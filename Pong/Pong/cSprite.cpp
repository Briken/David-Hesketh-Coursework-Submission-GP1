...
int cSprite::getTextureWidth()		//returns the texture width
{
	return textureWidth;
}
bool cSprite::isColliding()			//returns whether or not the object is colliding
{
	return colliding;
}
int cSprite::getPoints()			// returns the points value
{
	return points1;
}
void cSprite::registerCollision(bool collision) // sets whether ot not the object is colliding
{
	colliding = collision;
}
void cSprite::setPoints(int points)				// sets the points of the object
{
	points1 = points;
}